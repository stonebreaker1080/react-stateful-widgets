...
export default function Programmers() {
  ...
  const [ programmersList, setProgrammers ] = useState(listOfAwesome)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList.filter( (record) => record.id === highlightID)[0].name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setHighlight(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
